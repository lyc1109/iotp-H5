...
<template>
  <app-view :has-footer="!isUsrLease">
      ...
      <div class="bg-light full-height standard-lease-box">
        ...
        <div class="money-box margin-m margin-bottom-zero">
          ...
          <div class="bg-white padding-l money-detail">
            ...
            <div class="font-xl font-bold-700 align-center margin-top margin-bottom-l">{{ Number(cardVal/100).toFixed(2) }}</div>
          </div>
        ...
        <div class="pay-detail-box border-bottom margin-m margin-top-zero bg-white">
          <div class="valign-center padding-l flex">
            <div class="align-left flex-item">
              ...
              <div class="margin-bottom-m flex">
                <span class="font-light font-s">产品押金：</span><span class="font-bold-500 flex-item align-right">¥ {{ Number(rechargeList.deviceDeposit/100).toFixed(2) }}</span>
              </div>
              <div class="margin-bottom-m flex">
                <span class="font-light font-s">产品租金：</span><span class="font-bold-500 flex-item align-right">¥ {{ Number((Number(cardVal) - Number(rechargeList.deviceDeposit))/100).toFixed(2) }}</span>
              </div>
              <div class="margin-bottom-m flex">
    ...
    <div class="footer border-top flex valign-center" slot="footer" v-if="!isUsrLease">
      ...
      <!--<div v-if="isAlipay && leaseSet && leaseSet.supportCreditRent"-->
           <!--class="valign-center padding-left-m padding-right-m color-primary font-bold-500" style="border-left: 1px solid #58B7FF;">免押金支付</div>-->
  ...
  export default {
    ...
    created() {
      ...
      if (/iphone|ipad|ipod/.test(_ua)) {
        ...
        this.$store.commit('isRefresh',true);
      } else {
        this.$store.commit('isRefresh',true);
      }
    },
      ...
      fetchData(isFirst) {
        ...
        let initRent = function(_res,_type) {
          ...
          if(isFirst) {
            ...
            if(_credit !== '' && _credit !== undefined && _credit !== null) {
              _credit = _credit.toString();
              if(_credit === 'true') {
                _flag = true;
...
//                        that.annualCard = i;
        ...
        if(this.isOrderPay) {
          // 获取订单费用详情
          that.$http.get(`${that.$apihost}/serverorders/${that.$route.params.id}/fees`)
            .then((res) => {
              let _temp = res;
              for(let i = 0,len = res.length; i < len; i++) {
                let _type = _temp[i].feeType;
                if(_type === 'YJ') {
                  that.rechargeList.deviceDeposit = _temp[i].feeItemPrice;
                }
                if(_type === 'ZJ') {
                  that.cardVal = _temp[i].feeItemPrice;
                }
              }
              that.cardVal += Number(that.rechargeList.deviceDeposit);
            })
        ...
        if(this.$route.path.indexOf('serviceorder') > -1) {
          // 个人租赁支付押金
          this.isFirst  = true;
          this.$http.get(`${this.$apihost}/serverorders/${that.entityId}`)
            .then((res) => {
              this.entity = res;
              ...
              .then((res) => {
                if (res) {
                  ...
                  if(_chargeType === 'OD') {
                    ...
                    that.cardVal = that.rentList[_index].valueAfter;
                  }
                  that.pay();
                } else {
