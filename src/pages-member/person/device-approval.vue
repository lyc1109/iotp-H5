...
<template>
  <div v-if="approval && approval.length > 0" class="full-height">
    <div class="ouath-box bg full-height padding-bottom-xxl">
      <div v-for="(item, index) in approval"
          ...
          <div class="product-info-box">
            <div class="product-info flex-item flex">
              <div class="font-m font-title text-wrapper">
                <span class="order-productname font-bold-400">{{ item.device && item.device.deviceName ? item.device.deviceName : '无产品名称' }}</span>
              </div>
  ...
  export default {
    ...
    created() {
      ...
      this.$http.get(`${this.$apihost}/userdevices/pending-approvals`)
        .then((data) => {
        // 伪数据
//          this.approval = data.concat(data.concat(data));
            this.approval = data;
        })
    },
    methods: {
