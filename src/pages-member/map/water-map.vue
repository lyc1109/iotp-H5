...
<template>
  <div class='wrapper'>
    <baidu-map id="allmap" MapType="BMAP_SATELLITE_MAP" :min-zoom=3 :max-zoom=19  @ready="handler">
      <!-- -->
      <!--<bm-geolocation anchor="BMAP_ANCHOR_BOTTOM_RIGHT" :autoLocation="true"></bm-geolocation>-->
...
<script>
  ...
  import {mapStyleJson} from '@/assets/scripts/config-map'
  ...
  export default {
    ...
    methods: {
      handler({BMap, map}) {
        ...
        this.$map = map
        var that = this
        //   new BMap.Map({enableMapClick: false});
        // 根据浏览器定位
        //  this.getLocation()
...
//          span2.style.display = 'none'
      ...
      Anchor() {
        ...
        if(this.address.lng) {
          ...
          var point = new this.$BMap.Point(this.address.lng, this.address.lat)
