...
<template>
  <div v-infinite-scroll="loadMore"
    ...
    <v-spinner v-show="isLoading" :isAllPage="false"></v-spinner>
    ...
    <div v-if="totalElements !== '' && totalElements <= 0">
      <app-empty-view :type="type"></app-empty-view>
    </div>
  </div>
</template>
...
<script>
  ...
  export default{
    name: "app-page",
    components: {AppEmptyView, VSpinner},
    props: {
      // [必须] 分页数据请求URL
      url: {
        type: String,
    ...
    methods: {
      // 加载更多数据
      loadMore: function () {
        if ((this.totalPages > 0 && (this.curPage > this.totalPages)) || this.totalElements === 0) {
          this.isLoading = false;
