    ...
    <div class="popup-window-content">
              ...
              <div class="product-footer flex">
                <div v-if="servicesData">
                  ...
                  <div v-if="!servicesData.afterWarrantyPeriod" class="font-bold-400 valign-center" >
                    <del class="font-light"><span>{{ $filters.formatCurrency(item.formatPriceAfter) }}</span></del>
                    <span class="color-primary currency font-m">{{ $filters.formatCurrency(item.formatPrice) }}</span>
                  </div>
                  <!--已过保修期-->
                ...
                <div class="product-footer flex">
                  <div v-if="servicesData">
                    ...
                    <div v-if="!servicesData.afterWarrantyPeriod" class="color-primary font-bold-400 valign-center" >
                      <del class="font-light"><span>{{ $filters.formatCurrency(item.overduePrice/100) }}</span></del>
                      <span class="color-primary currency font-m">{{ $filters.formatCurrency(item.price/100) }}</span>
                    </div>
                    <!--已过保修期-->
                    <div v-else-if="servicesData.afterWarrantyPeriod">
                      <span class="color-primary currency font-m">{{ $filters.formatCurrency(item.price) }}</span>
                    </div>
                  </div>
                  <div class="flex-item"></div>
                  <div class="product-number">
            ...
            <div class="flex checklist-box"
              ...
              <div class="mint-cell-checkbox">
                <input type="checkbox"
                       ...
                       v-model="feeData.pjCheck"
                       @click="addService({
                       feeItemPrice: (servicesData && !servicesData.afterWarrantyPeriod ? item1.price : item1.overduePrice),
                       feeType: 'SPEC',
                       feeDefId: 0,
                       feeDefConfuseId: item1.id,
                       feeItemName: item.name,
                       feeItemNum: feeData.pjQuatityNum[item1.id],
                       subTotal: (servicesData && !servicesData.afterWarrantyPeriod ? item1.price : item1.overduePrice) * feeData.pjQuatityNum[item1.id]
                       },item1.itemCode)"/>
                <i class="mint-checkbox-core"></i>
              </div>
              <div class="product-logo border border-radius valign-center">
              ...
              <div class="flex-item flex product-content">
                ...
                <div class="product-footer flex">
                  <div v-if="servicesData">
                    <!--保修期内-->
                    <div v-if="!servicesData.afterWarrantyPeriod" class="color-primary font-bold-400 valign-center" >
                      <del class="font-light"><span>{{ $filters.formatCurrency(item1.overduePrice/100) }}</span></del>
                      <span class="color-primary currency font-m">{{ $filters.formatCurrency(item1.price/100) }}</span>
                    </div>
                    <!--已过保修期-->
                    <div v-else-if="servicesData.afterWarrantyPeriod">
                      <span class="color-primary currency font-m">{{ $filters.formatCurrency(item1.price) }}</span>
                    </div>
                  </div>
                  <div class="flex-item"></div>
                  <div class="product-number">
      ...
      <mt-popup v-model="feeDetailPopup"
        ...
        <div class="select-box bg-white">
          <!--详情弹窗页顶部-->
          <div class="padding-m font-s flex select-title-box">
...
//        this.$store.commit(types.wos.setServiceData, '');
        ...
        if (this.dataType === 'service') {
          ...
          if (_checkLen < _fwItemLen && !curserviceArr.includes(items.feeDefConfuseId)) {
          ...
          } else if (_checkLen === _fwItemLen && curserviceArr.includes(items.feeDefConfuseId)) {
          ...
          if (hasin) {
            if (!(_checkLen > 0 && curserviceArr.includes(items.feeDefConfuseId))) {
          ...
          if (pjItemArr && _pjItemLen > 0) {
            if (_checkLen < _pjItemLen && !curserviceArr.includes(items.feeDefConfuseId)) {
            ...
            } else if (_checkLen === _pjItemLen && curserviceArr.includes(items.feeDefConfuseId)) {
              curserviceArr.forEach((item, index) => {
                if (item === items.feeDefConfuseId) {
                  hasin = false;
      ...
      deletSelect(_curItemArr, _curCheckArr, index,itemid, i, len) {
        ...
        this.$messagebox({
          ...
          if(action === 'confirm') {
            ...
            if(len >= 2) {
              this.calculate('dec', _curItemArr[_curIndex].feeDefConfuseId, i);
            }
          } else {
            if(this.dataType === 'service') {
              this.feeData.fwQuatityNum[index] = 1;
              this.feeData.fwQuatityNum.splice(index, 1, this.feeData.fwQuatityNum[index]);
            } else if(this.dataType === 'fitting') {
              this.feeData.pjQuatityNum[index] = 1;
              this.feeData.pjQuatityNum.splice(index, 1, this.feeData.pjQuatityNum[index]);
            }
          }
        });
      },
