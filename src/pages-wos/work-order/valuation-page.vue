      ...
      <div class="padding-m flex order-info-item bg-white">
        <div class="flex-item-2 margin-right-m">
          <div class="valuation-image valign-center border border-radius">
            <img v-lazy="$filters.img((entity.productImage ? entity.productImage : ''),'!q70')">
          </div>
        </div>
    ...
    methods: {
      ...
      confirmFee() {
        this.$messagebox({
          ...
          if(action === 'confirm') {
            let _params = {
              'adjust_fee': parseInt(this.totalFee * 100),
              'recalculate_fee': true,
              'fee_items': JSON.stringify(this.feeData.fwItems.concat(this.feeData.pjItems))
            };
            this.$http.post(this.$woshost + '/workorders/' + this.entityId + '/fee/calculate',_params)
              .then((res) => {
                this.$store.commit(types.wos.setServiceData, null);
                this.$store.commit(types.wos.setPageData, null);
                this.$toast.success('计价成功');
                this.$router.go(-1);
            });
          }
        });
