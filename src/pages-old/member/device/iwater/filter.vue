  ...
  <app-view :has-footer="true" class="bg-white">
    <div class="filter-detail bg-white">
      <div class="filter-img valign-center">
        <div :style="{ backgroundImage: `url(${$filters.img(availableFilter.productImage,'!wh400')})` }"></div>
      </div>
    ...
    <div class="footer padding" slot="footer">
      ...
      <div class="available-operate">
        <button type="button" class="btn btn-link btn-sm" @click="cancel()">取消</button>
        <button type="button" class="btn btn-primary btn-round btn-sm" @click="filterReset()">滤芯复位</button>
      </div>
    </div>
  </app-view>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 初始化数据
      fetchData(){
        this.availableFilter.value = Number(this.$route.query.availablePercentage)
      ...
      // 取消复位
      cancel(){
        MessageBox.confirm('确定取消滤芯复位?').then(() => {
          this.$http.post(`${this.$apihost}/waterdevices/${this.$route.params.id}/unresetFilter/${this.$route.query.index + 1}`)
            .then((res) => {
              this.availableFilter.value = Number(res.availablePercentage)
              ...
              Messenger.success('已取消复位')
            })
        })
      },
      // 滤芯复位
      filterReset(){
        MessageBox.confirm('确定进行滤芯复位?').then(() => {
          this.$http.post(`${this.$apihost}/waterdevices/${this.$route.params.id}/resetFilter/${this.$route.query.index + 1}`)
            .then((res) => {
